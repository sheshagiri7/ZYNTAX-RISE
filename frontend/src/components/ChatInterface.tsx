    ...
    <div className="chat-interface" style={{ marginTop: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {response && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {response.grounded ? (
            <div style={{ background: 'rgba(16, 185, 129, 0.1)', color: '#10b981', padding: '10px 14px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 'bold', fontSize: '0.9rem' }}>
              <span>✓</span> GROUNDED: TRUE
            </div>
          ) : (
            <div style={{ background: 'rgba(255, 170, 0, 0.1)', color: '#ffaa00', padding: '12px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 'bold' }}>
              <AlertTriangle size={20} /> GROUNDED: FALSE — NOT GROUNDED IN UPLOADED DATA
            </div>
          )}
